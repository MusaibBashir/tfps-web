...
import type { User as UserType, Equipment } from "../types"
  ...
  useEffect(() => {
    const fetchMemberDetails = async () => {
      ...
      try {
        ...
        const { data: memberData, error: memberError } = await supabase.from("users").select("*").eq("id", id).single()
  ...
  if (!member) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">The member you're looking for doesn't exist or has been removed.</p>
        <div className="mt-6">
          <Link
            to="/members"
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="p-6 sm:p-8 bg-primary-50">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            <div className="avatar h-24 w-24 text-2xl flex-shrink-0">{getInitial(member.name)}</div>
        ...
        <div className="p-6 sm:p-8 border-t border-gray-200">
          ...
          {equipment.length > 0 ? (
                  ...
                  {equipment.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <Link to={`/equipment/${item.id}`} className="text-primary-600 hover:text-primary-900">
                          View details
                        </Link>
                      </td>
                    </tr>
            ...
            <div className="text-center py-8 bg-gray-50 rounded-lg">
              ...
              <p className="mt-1 text-sm text-gray-500">This member doesn't own any equipment.</p>
            </div>
          )}
        </div>
      </div>
