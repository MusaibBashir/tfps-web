...
import { ArrowLeft, Package, User, Calendar, Clock, ChevronDown, ChevronUp, Plus } from 'lucide-react';
import { useSupabase } from '../contexts/SupabaseContext';
import { useAuth } from '../contexts/AuthContext';
import { Equipment, EquipmentLog, User as UserType } from '../types';
    ...
    <div className="container mx-auto max-w-5xl">
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className={`p-6 sm:p-8 ${
          equipment.status === 'available' ? 'bg-green-50' : 
          equipment.status === 'in_use' ? 'bg-yellow-50' : 'bg-red-50'
        }`}>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            <div className={`p-6 rounded-full bg-white ${
              equipment.status === 'available' ? 'text-green-500' : 
              equipment.status === 'in_use' ? 'text-yellow-500' : 'text-red-500'
            }`}>
            ...
            <div className="text-center sm:text-left">
              ...
              <div className="mt-4 flex flex-wrap gap-2 justify-center sm:justify-start">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  ...
                  {equipment.status.replace('_', ' ').charAt(0).toUpperCase() + equipment.status.replace('_', ' ').slice(1)}
                </span>
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  equipment.ownership_type === 'hall' 
        ...
        <div className="p-6 border-t border-gray-200">
          ...
          {history.length > 0 ? (
                    ...
                    <div className="ml-3 flex-1">
                      ...
                      {log.return_time && (
                        <div className="mt-1 text-xs text-gray-500">
                          Returned: {new Date(log.return_time).toLocaleDateString()} {new Date(log.return_time).toLocaleTimeString()}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="text-center py-8 bg-gray-50 rounded-lg">
              ...
              <p className="mt-1 text-sm text-gray-500">
                This equipment hasn't been used yet.
              </p>
            </div>
          )}
        </div>
